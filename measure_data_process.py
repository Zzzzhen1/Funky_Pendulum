...
dirc_path = r'C:\Programming\Python\Funky Pendulum\cart_pendulum_data\Natural_Freqency_Check\09-08-csv-owen'
...
def measure_fit_exp(time, angle):
    ...
    popt, pcov = curve_fit(damp_sin, time, angle, 
                           ...
                           bounds = ((0.1, 2*np.pi*1., -np.pi, 0., -0.2), (0.3, 2*np.pi*1.3, np.pi, np.inf, 0.2)),
                           maxfev = 200000000)
    return popt, pcov
...
def measure_plot(save = False, filename = 'measure.csv', block_plot = True):
    ...
    if(temp_index == 0):
        ...
        figure.suptitle("sampling rate = " + str(1000 * avg_spacing)[:6] + " ms")
        ...
        for i,j in zip(peaks, abs(fft_angle[peaks])):
            ...
            axes[1].text(1.25, 0.2, "total time = " + str(time[high_ind - 1] - time[low_ind])[:6] + " s")

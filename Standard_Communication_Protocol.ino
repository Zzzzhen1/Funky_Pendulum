...
// Therefore the corresponding microstep is 200 / STEPS, which needs to be taken into consider
...
// PID parameters
float Kp = 400;        //1500 Reasonable: 300-2000
float Ki = 400;        //1400 Reasonable: 300-1700
float Kd = 10;         //5 Reasonable: 2-10
float Kp_pos = 0.04;   //0.08 Reasonable: 0.001-0.1
float Ki_pos = 0.035;  //0.08 Reasonable: 0.001-0.1
float Kd_pos = 0.003;  //0.0035 Reasonable: 0.0001-0.005

// Define the button
#define Lbtn_pin 5        // Left switch pin
#define Rbtn_pin 4        // Right switch pin
ezButton Lbtn(Lbtn_pin);  // Left switch
ezButton Rbtn(Rbtn_pin);  // Right switch
...
void reset() {
  ...
  buf_ind = 0;
  temp_ind = 0;
  circ_buffer_angle[circ_buf_len] = { 0 };
  circ_buffer_time[circ_buf_len] = { 0 };
  circ_buffer_position[circ_buf_len] = { 0 };
  ang_eq = 0.;
  init_ang_cul = 0.;
  Serial.flush();
}
  ...
  if (Serial) {
    if (flag_swing_request) {
      ...
      if (flag_pid_input) {
        ...
        Serial.println("For example: 800,1800,5,0.08,0.007,0.002");
...
// TODO: need to fix the run_speed()
